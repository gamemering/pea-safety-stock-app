...
GOOGLE_SHEET_ID = "1QBq6Z8QVjiu9krz7rOCXicXPOHFBUUj04vHnlJsddt8"
...
# พจนานุกรมชื่อคลังที่เป็นมิตรภาษาไทย
WAREHOUSES_MAP = {
    "C010": "คลังลพบุรี",
    "C020": "คลังนครสวรรค์",
    "C030": "คลังอุทัยธานี",
...
def parse_mb52_txt(file_content):
    ...
    for line in lines:
        ...
        if match_mat:
            ...
            # Extract description
            remaining = line[len(raw_code):].strip()
            ...
            if tokens[3] in ['EA', 'KG', 'M', 'PAC', 'L']:
                ...
                try:
                    ...
                    if current_material:
                        ...
                        material_data[current_material]['SLocs'][sloc_id] = material_data[current_material]['SLocs'].get(sloc_id, 0.0) + qty
                        all_slocs.add(sloc_id)
                except ValueError:
                    pass

    # Ensure Qty_0021 is always present so safety stock alert logic doesn't break
    all_slocs.add("0021")
    sorted_slocs = sorted(list(all_slocs))
    columns = ['SAP_Code', 'Description', 'Actual_Qty'] + [f'Qty_{sloc}' for sloc in sorted_slocs]

    parsed_list = []
    for sap_code, data in material_data.items():
        row = [sap_code, data['Description'], data['Total_Qty']]
        for sloc in sorted_slocs:
            row.append(data['SLocs'].get(sloc, 0.0))
        parsed_list.append(row)

    df_parsed = pd.DataFrame(parsed_list, columns=columns)
    return df_parsed
...
# ═══════════════════════════════════════════════════════════════════════
#  SIDEBAR — เลือกคลัง + อัปโหลด MB52 (จุดควบคุมเดี่ยวครบวงจร)
# ═══════════════════════════════════════════════════════════════════════
if df_safety is not None:
    ...
    warehouse_option = st.sidebar.selectbox(
        ...
        format_func=lambda x: f"{x} - {WAREHOUSES_MAP.get(x, '')}",
    ...
    uploaded_mb52 = st.sidebar.file_uploader(
        f"ลากวางไฟล์ MB52.txt ของคลัง [{warehouse_option} - {WAREHOUSES_MAP.get(warehouse_option, '')}] ที่นี่",
            ...
            if not df_parsed.empty:
                with st.spinner(f"กำลังอัปเดตฐานข้อมูลคลัง {warehouse_option} ลง Google Sheets..."):
                    ...
                    if client is not None:
                        ...
                        # 1. บันทึกข้อมูลดิบ MB52 ตรงเข้าคลังที่เลือกอยู่ ณ ปัจจุบัน
                        try:
                        ...
                        if 'Description' in df_parsed_line.columns:
                            df_parsed_line = df_parsed_line.drop(columns=['Description'])
            ...
            if client is not None:
                try:
                    ...
                    if records:
                        ...
                        if 'Description' in df_mb52_resend.columns:
                            df_mb52_resend = df_mb52_resend.drop(columns=['Description'])
    ...
    # ═══════════════════════════════════════════════════════════════════
    st.write(f"📊 กำลังแสดงยอดเปรียบเทียบคลัง: **{warehouse_option} - {WAREHOUSES_MAP.get(warehouse_option, '')}**")
    ...
    if client is not None:
        try:
            ...
            if records:
                ...
                # Ensure Qty_0021 and Actual_Qty exist in df_mb52_clean to avoid KeyError
                if 'Qty_0021' not in df_mb52_clean.columns:
                    df_mb52_clean['Qty_0021'] = 0.0
                if 'Actual_Qty' not in df_mb52_clean.columns:
                    df_mb52_clean['Actual_Qty'] = 0.0
        except gspread.exceptions.WorksheetNotFound:
            df_mb52_clean = None
        except Exception:
            df_mb52_clean = None
    ...
    if df_mb52_clean is not None and not df_mb52_clean.empty:
        ...
        if 'Description' in df_mb52_clean.columns:
            df_mb52_clean = df_mb52_clean.drop(columns=['Description'])
        ...
        df_result['ลำดับ'] = df_merge['No'] if 'No' in df_merge.columns else range(1, len(df_merge) + 1)
