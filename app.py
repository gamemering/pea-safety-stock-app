...
# --- 3. แกะไฟล์เกณฑ์ Safety Stock จริง (ขยาย 100 คอลัมน์กันพลาด) ---
def parse_safety_stock_csv(file_content):
    try:
        ...
        for idx, row in raw_df.iterrows():
            ...
            # กรองเฉพาะแถวรหัสพัสดุ 10 หลัก ตัดแถวหัวข้อภาษาไทยทิ้ง
    ...
    try:
        ...
        with st.spinner("กำลังแกะโครงสร้างไฟล์จริงและบันทึกขึ้นระบบคลาวด์ถาวร..."):
            ...
            if df_safety_parsed is not None and not df_safety_parsed.empty:
                ...
                # ขยายขนาดชีตอัตโนมัติป้องกัน Out of bounds
...
if df_safety is not None and not df_safety.empty:
        ...
        with st.spinner(f"กำลังส่งไลน์รายงานคลัง {warehouse_option}..."):
            try:
                ...
                if records:
                    ...
                    df_s_resend['SAP_Code'] = df_s_resend['SAP_Code'].astype(str).str.strip()
                    df_resend['SAP_Code'] = df_resend['SAP_Code'].astype(str).str.strip()
                    
                    df_m = pd.merge(df_s_resend, df_resend, on='SAP_Code', how='left')
                    df_m['Qty_0021'] = pd.to_numeric(df_m['Qty_0021'], errors='coerce').fillna(0)
                    df_m[warehouse_option] = pd.to_numeric(df_m[warehouse_option], errors='coerce').fillna(0)
    ...
    # 🛡️ จุดป้องกัน Error รหัสพัสดุ! (แปลงให้เป็นตัวเลข 100%)
    ...
    if df_mb52_clean is not None and not df_mb52_clean.empty:
        ...
        df_result = pd.DataFrame({
            ...
            'จำนวนอุปกรณ์ในคลัง (รวมทุก SLoc)': pd.to_numeric(df_merge['Actual_Qty'], errors='coerce').fillna(0).astype(int),
            'จำนวนอุปกรณ์ในคลัง (เฉพาะ 0021)': df_merge['Qty_0021'].astype(int),
            'อนุมัติ safety stock': pd.to_numeric(df_merge[warehouse_option], errors='coerce').fillna(0).astype(int)
        })
    ...
    st.info("⚠️ กรุณาลากวางไฟล์เกณฑ์จริง (.csv) ที่แผงควบคุมด้านซ้ายเพื่อเปิดใช้งานระบบหลัก")
